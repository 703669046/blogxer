...
import { getListPage, handleCollect } from "./api/index";
...
export default {
    ...
    methods: {
        ...
        async handleCollects(row) {
            if (row.collect == 0 || row.collect == null) {
                row.collect = 1;
            } else {
                row.collect = 0;
            }
            let param = {
                collectType: Boolean(row.collect),
                post_id: row.post_id,
